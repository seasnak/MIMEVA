            ...
            while (!sr.EndOfStream)
            {
                ...
                switch (output_arr[0])
                {
                    ...
                    case "ROOM":
                        int depth = 0;
                        output_arr = sr.ReadLine().Split(' ');
                        while (depth < level.shape[0] && output_arr.Length != 0)
                        { // append room part to level
        ...
        */
        ...
        for (int i = 0; i < this.shape[0]; i++)
        {
            for (int j = 0; j < this.shape[1]; j++)
            {
                ...
                { // TODO: change to have a variable value for left connector

                }
                else if (val == "R")
                { // TODO: change to have a variable value for right connector
