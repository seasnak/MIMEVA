...
public partial class Fleep : Enemy
{
    ...
    [Export] protected int keepaway_dist = 20; // the minimum distance fleep keeps away from the player
    ...
    protected override void SetMovementLogic()
    {
        ...
        Godot.Vector2 vec_to_player = this.GlobalPosition - player.GlobalPosition;
        Random random = new();
        if (vec_to_player.Length() > alert_range)
        { // passive movement
            is_passmove = true;
        ...
        else if (vec_to_player.Length() > keepaway_dist && !is_backingup)
        { // regular movement
